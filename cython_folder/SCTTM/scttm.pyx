"""Supervised Correspondence Topic Tracking Model with Cython"""
...
cdef class SCTTM():
    ...
    # ハイパラ保存用
    cdef public double[:,:] alpha_list
    cdef public list mse_list
    cdef public list r_score
    cdef public double mse, r

    # フラグが True の時のみ月齢パラメータを更新
    cdef bool flag
    ...
    def __init__(self, unsigned int K, unsigned int L, vector[vector[vector[int]]] N_tdv,
                ...
                double[:] eta_k = None, double sigma = -1.0):
        """ 初期化 """
        # N : T×D×V
        ...
        self.alpha_tdl = np.full((self.T, self.D, self.L), 0.2) # 一様
        self.beta_tkl = np.full((self.T, self.K, self.L), 0.2) # 一様

        # トピック分布, 単語分布, 補助情報分布の初期化
        self.theta_tdk = np.full((self.T, self.D, self.K), 0.0)
        self.phi_tkv = np.full((self.T, self.K, self.V), 0.0)
        self.psi_sex_tks = np.full((self.T, self.K, self.S), 0.0)
        self.psi_chi_tks = np.full((self.T, self.K, self.Chi), 0.0)
        self.psi_gen_tks = np.full((self.T, self.K, self.Gen), 0.0)
        self.psi_pre_tks = np.full((self.T, self.K, self.Pre), 0.0)
        self.psi_q_tks = np.full((self.T, self.K, self.Q), 0.0)
        ...
        # 時刻 t, トピック k の月齢に関する偏回帰係数のベクトル
        if eta_k is None:
            self.flag = True
            self.eta_k = np.linspace(-1, 1, self.K) # [-1, 1] を 2/K 刻みで初期化
        else:
            self.flag = False
            self.eta_k = eta_k
        # 時刻 t の月齢に関する分散パラメータ
        if sigma < 0:
            self.flag = True
            self.sigma = np.var(np.array(self.Age_td[0]))
        else:
            self.flag = False
            self.sigma = sigma
        # 回帰の説明変数の初期化
    ...
    def scttm_estimate(self, unsigned int iteration, unsigned int sample_size):
        """崩壊型ギブスサンプリングで推定"""

        cdef unsigned int t, d, k, l, _l, v, s, num_chi, gen_id, pre_id, q_id, index
        cdef unsigned int itr
        cdef int topic, new_topic
        cdef double tmp_alpha_td, tmp_beta_tk, tmp_sum, sigma_left
        cdef double[:] tmp_first_alpha, pred_y, eta_right
        ...
        cdef double alpha_theta, beta_phi, p_exp_age
        ...
        self.alpha_list = np.zeros(shape=(self.K, iteration))
        ...
        for t in range(self.T):
            ...
            for itr in tqdm(range(iteration)):
                        ...
                        for num in range(self.N_tdv[t][d][v]):
                            ...
                            if t == 0:
                                for k in range(self.K):
                                    ...
                                    p_exp_age = c_exp((self.eta_k[k]/(self.N_td[t][d]*self.sigma))*(self.Age_td[t][d] - np.dot(self.eta_k, self.z_tdk_var[t][d]) - (self.eta_k[k]/(2*self.N_td[t][d]))))
                                    self.p_ztd_n[t][k] = p_alpha * (p_beta_numer / p_beta_denom) * p_aid * p_exp_age
                                    # np.inf の例外処理 (主にテストデータの評価で発生)
                                    if self.p_ztd_n[t][k] == float('inf'): self.p_ztd_n[t][k] = 1.0
                                # end for k
                ...
                if t == 0:
                    ...
                    """補助情報: 子供の月齢に関する回帰パラメータの推定"""
                    # eta の更新
                    eta_right = np.zeros(shape=(self.K))
                    eta_left = np.zeros(shape=(self.K, self.K))
                    sigma_left = 0.0
                    del_list = []
                    pred_y = np.zeros(shape=(self.D))
                    ...
                    if self.flag:
                        for d in range(self.D):
                            ...
                            for k in range(self.K):
                                self.z_tdk_var[t][d][k] = self.N_tdk[t][d][k]/(self.N_td[t][d] * (itr+1.0))
                            # end for k
                            eta_right += np.dot(self.Age_td[t][d], self.z_tdk_var[t][d])
                            eta_left += np.outer(self.z_tdk_var[t][d], self.z_tdk_var[t][d])
                            sigma_left += c_pow(self.Age_td[t][d], 2)
                        # end for d
                        eta_temp = np.linalg.pinv(eta_left)@eta_right
                        for k in range(self.K):
                            self.eta_k[k] = eta_temp[k]
                        
                        # sigma の更新
                        self.sigma = (sigma_left - np.dot(self.eta_k, eta_right))/(self.D - len(del_list))
                    # end if

                    # モデル評価
                    mse = 0.0
                    for d in range(self.D):
                        pred_y[d] = np.dot(self.eta_k, self.z_tdk_var[t][d])
                        if self.N_td[t][d] == 0: continue
                        mse += c_pow(self.Age_td[t][d] - pred_y[d], 2)
                    # end for
                    self.mse_list.append(mse/(self.D - len(del_list)))
                    self.r_score.append(r2_score(np.delete(self.Age_td[t], del_list), np.delete(pred_y, del_list)))

                    
                    if itr == iteration - 1:
                        self.mse = mean_squared_error(np.delete(self.Age_td[t], del_list), np.delete(pred_y, del_list))
                        self.r = r2_score(np.delete(self.Age_td[t], del_list), np.delete(pred_y, del_list))
                    # end if
                # end if
                ...
                if t == 0:
                    ...
                    for k in range(self.K):
                        ...
                        try:
                            ...
                            self.alpha_list[k][itr] = self.first_alpha[k]
                ...
                # 最初から数えて L 時刻目までの処理
                    ...
                    for d in range(self.D):
                        ...
                        # 時刻 t で投稿数 0 だと alpha と theta は更新しない
                        if self.N_td[t][d] == 0:
                            continue
